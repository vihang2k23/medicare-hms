        ...
        {isAuthenticated ? (
          <>
            ...
            <div className="relative ml-1 sm:ml-2" ref={ref}>
              ...
              >
                <div
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800/90 text-sm font-semibold text-slate-700 dark:text-slate-200 ring-1 ring-slate-200/90 dark:ring-slate-700/80"
                  aria-hidden
                >
                  {user?.name?.charAt(0) ?? '?'}
                </div>
                <div className="hidden min-w-0 flex-1 flex-col items-stretch text-left sm:flex">
                  ...
                  <span className="mt-1 text-[11px] font-medium leading-none text-slate-500 dark:text-slate-400">
                    {ROLE_DISPLAY[user?.role ?? 'admin']}
                  </span>
                </div>
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition-colors dark:bg-slate-800/70 dark:text-slate-400 group-hover:bg-slate-100 dark:group-hover:bg-slate-700/80">
                  <ChevronDown
                    strokeWidth={2.5}
              ...
              {dropdownOpen && (
                ...
                >
                  <div className="flex items-center gap-3 rounded-xl bg-slate-50/90 px-3 py-3 dark:bg-slate-800/50">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white text-base font-semibold text-slate-700 ring-1 ring-slate-200/90 dark:bg-slate-800 dark:text-slate-200 dark:ring-slate-600/80">
                      {user?.name?.charAt(0) ?? '?'}
                    </div>
                  ...
                  <p className="px-3 pt-2 pb-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">
                    Switch account
                  </p>
                  <div className="max-h-52 overflow-auto px-0.5 pb-1">
                    {ALL_DEMO_LOGIN_ENTRIES.map((u) => (
                      ...
                      >
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                          {u.name.charAt(0)}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-medium text-slate-900 dark:text-white">{u.name}</p>
                          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{ROLE_DISPLAY[u.role]}</p>
                        </div>
                      </button>
                    ))}
                  </div>
                  <div className="border-t border-slate-200/80 p-1.5 dark:border-slate-700/80">
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-red-600 transition-colors hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40"
                    >
